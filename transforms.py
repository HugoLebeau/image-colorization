...
def rgb2lab(img_rgb):
    ...
    -------
    torch.tensor
        (3, H, W) image in the L*a*b* color space.
    ...
    # XYZ > L*a*b*
    X, Y, Z = X/0.95047, Y, Z/1.08883 # CIE Standard Illuminant D65, 2° observer
    img_xyz = torch.stack([X, Y, Z])
...
def lab2rgb(img_lab):
    ...
    img_xyz[0], img_xyz[1], img_xyz[2] = img_xyz[0]*0.95047, img_xyz[1], img_xyz[2]*1.08883 # CIE Standard Illuminant D65, 2° observer
    # XYZ > RGB
...
def rgb2l(img_rgb):
    ...
    Y = img[0]*0.212671+img[1]*0.715160+img[2]*0.072169
    # Y > L*
    Y = Y # CIE Standard Illuminant D65, 2° observer
    mask = Y > 0.008856
    Y[mask] = Y[mask]**(1./3.)
...
data_transform = {
    'gray': transforms.Compose([transforms.ToTensor(),
                                (lambda img_rgb: rgb2l(img_rgb).view(1, -1, img_rgb.shape[-1]))
                                ]),
    'color': transforms.Compose([transforms.ToTensor()
                                 ])}
